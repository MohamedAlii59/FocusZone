...
BEGIN
    ...
    SELECT @QuizScore = ISNULL(AVG(Score), 0)
    FROM   Evidence
    WHERE  SessionId = @SessionId
      AND  TopicId   = @TopicId
      AND  Type IN ('quiz', 'assessment');

    SELECT @StudyScore = ISNULL(AVG(Score), 0)
    FROM   Evidence
    WHERE  SessionId = @SessionId
      AND  TopicId   = @TopicId
      AND  Type = 'study_time';

    SELECT @RetentionScore = ISNULL(AVG(Score), 0)
    FROM   Evidence
    WHERE  SessionId = @SessionId
      AND  TopicId   = @TopicId
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM UserTopicMastery
        WHERE UserId = @UserId AND TopicId = @TopicId
    )
        INSERT INTO UserTopicMastery (UserId, TopicId, Mastery, Confidence, Interest, EvidenceCount)
        VALUES (@UserId, @TopicId, 0.00, 0.00, 0.50, 0);
...
BEGIN
    ...
    SELECT
        @StdDev   = ISNULL(STDEV(E.Score), 0),
        @AvgScore = ISNULL(AVG(E.Score),   0)
    FROM Evidence      E
    JOIN StudySessions SS ON SS.SessionId = E.SessionId
    WHERE SS.UserId  = @UserId
      AND E.TopicId  = @TopicId;
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM UserTopicMastery WHERE UserId = @UserId AND TopicId = @TopicId
    )
        INSERT INTO UserTopicMastery (UserId, TopicId, Mastery, Confidence, Interest, EvidenceCount)
        VALUES (@UserId, @TopicId, 0.00, 0.00, 0.50, 0);
END;
...
BEGIN
    ...
    BEGIN
        INSERT INTO Resources (Title, Type, Url, Difficulty, Depth, EstimatedMinutes)
        VALUES (@Title, @Type, @Url, @Difficulty, @Depth, @EstimatedMinutes);
