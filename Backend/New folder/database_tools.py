"""
Updated database_tools.py for merged AspNetDatabase
...
def create_study_session(user_id: str, resource_id: Optional[int], summary: str, duration_minutes: float) -> dict:
    """Create a StudySessions row and return the session id. Now uses UserId (string) from AspNetUsers."""
    conn = get_conn()
    cursor = conn.cursor()
...
def fetch_user_goals(user_id: str) -> List[str]:
    """Fetch user goals (merged from Goals table)."""
    conn = get_conn()
    try:
        cur = conn.cursor()
...
def fetch_projects(user_id: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Fallback to simpler projection if some columns don't exist
        try:
            cur = get_conn().cursor()
...
def fetch_unmastered_topics(user_id: str) -> List[Dict[str, Any]]:
    """Return topics for which the user has no UserTopicMastery entry."""
    conn = get_conn()
    try:
        cur = conn.cursor()
        cur.execute(
...
def fetch_resource_topics(resource_id: int) -> List[str]:
    ...
    try:
        ...
        try:
            cur.execute(
                "SELECT t.Name FROM ResourceTopicCoverage rtc JOIN Topics t ON rtc.TopicId = t.TopicId WHERE rtc.ResourceId = ?",
                resource_id,
            )
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT t.Name FROM ResourceTopics rt JOIN Topics t ON rt.TopicId = t.TopicId WHERE rt.ResourceId = ?",
                resource_id,
            )
            return [r[0] for r in cur.fetchall()]
    finally:
